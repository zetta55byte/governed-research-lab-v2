...
export default function App() {
  ...
  // Live SSE connection — fires whenever sessionId changes
      ...
      <div style={{ display: 'grid', gridTemplateRows: '48px 1fr 72px', height: '100vh', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '272px 1fr 280px', overflow: 'hidden', minHeight: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', borderLeft: '1px solid #1e2a3a',
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid #1e2a3a', flexShrink: 0 }}>
              {RIGHT_TABS.map(({ id, label }) => (
                <button key={id} onClick={() => setRightTab(id)} style={{
                  flex: 1, padding: '6px 0', border: 'none', fontFamily: 'Syne',
                  fontWeight: 700, fontSize: 9, letterSpacing: 1.5, textTransform: 'uppercase',
                  cursor: 'pointer',
            ...
            <div style={{ flex: 1, overflow: 'hidden', minHeight: 0 }}>
              {rightTab === 'chain'        && <ContinuityChain deltas={state.continuityChain} />}
        ...
        {showBrief && state.finalBrief && (
          <div style={{
            ...
            zIndex: 50, display: 'flex', flexDirection: 'column', padding: 24, overflow: 'hidden',
          }}>
            <div style={{ display: 'flex', alignItems: 'center',
              justifyContent: 'space-between', marginBottom: 16 }}>
              <span style={{ fontFamily: 'Syne', fontSize: 16, fontWeight: 800 }}>
                ★ Final Research Brief
              </span>
              <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                <span style={{ fontSize: 10, color: '#64748b', fontFamily: 'Space Mono' }}>
                  {state.continuityChain.length} deltas ·{' '}
                  {state.membraneLog.length} membrane checks ·{' '}
                  S(t) = {state.currentStability.toFixed(3)}
                </span>
                <button onClick={() => setShowBrief(false)} style={{
                  background: '#131929', border: '1px solid #1e2a3a', borderRadius: 4,
                  padding: '5px 12px', color: '#e2e8f0', fontFamily: 'Space Mono',
                  fontSize: 11, cursor: 'pointer',
                }}>Close</button>
              </div>
            </div>
            <div style={{
              flex: 1, overflowY: 'auto', fontSize: 13, lineHeight: 1.9,
              color: '#e2e8f0', whiteSpace: 'pre-wrap', background: '#131929',
