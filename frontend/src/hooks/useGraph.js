...
const VERDICT_COLORS = { allow: '#10b981', block: '#ef4444', defer: '#f59e0b' };
...
export function useGraph(svgRef, graph) {
  ...
  useEffect(() => {
    ...
    nodeGrp.append('circle').attr('r',18).attr('fill',d=>ROLE_COLORS[d.role]||'#fff').attr('opacity',.08);
    ...
    nodeGrp.append('text').attr('dy',26).attr('text-anchor','middle').attr('font-size','9px')
      .attr('fill',d=>ROLE_COLORS[d.role]||'#fff').attr('opacity',.8).text(d=>d.label||d.id);
