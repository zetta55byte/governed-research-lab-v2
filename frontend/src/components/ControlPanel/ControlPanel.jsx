...
const STATUS_COLOR = {
  idle: '#334155', running: '#3b82f6', done: '#10b981', blocked: '#ef4444', deferred: '#f59e0b',
};

const AGENT_IDS = [
  'planner', 'researcher_1', 'researcher_2', 'researcher_3', 'critic', 'synthesizer',
];
...
export default function ControlPanel({ state, dispatch }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%',
      overflowY: 'auto', background: '#0d111a', borderRight: '1px solid #1e2a3a' }}>
      ...
      <div style={{ padding: '10px 12px', borderBottom: '1px solid #1e2a3a' }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {RUNTIMES.map(r => {
            ...
            return (
              <button key={r.id} onClick={() => dispatch({ type: 'SET_RUNTIME', runtime: r.id })}
                disabled={isRunning}
                style={{
                  flex: 1, padding: '8px 6px', borderRadius: 6, cursor: isRunning ? 'not-allowed' : 'pointer',
        ...
        {/* Runtime trajectory note */}
        <div style={{ marginTop: 6, fontSize: 9, color: '#334155', fontStyle: 'italic', textAlign: 'center' }}>
      ...
      <div style={{ padding: '10px 12px', borderBottom: '1px solid #1e2a3a' }}>
        <div style={{ fontSize: 9, color: '#334155', letterSpacing: 2,
          textTransform: 'uppercase', fontFamily: 'Syne', fontWeight: 700, marginBottom: 6 }}>
          Query
        </div>
        <textarea
          value={state.query}
          ...
          style={{
            width: '100%', background: '#131929', border: '1px solid #1e2a3a',
            borderRadius: 6, padding: '8px 10px', color: '#e2e8f0',
            fontFamily: 'Space Mono', fontSize: 11, resize: 'vertical',
            minHeight: 60, outline: 'none', transition: 'border-color .15s',
          }}
      ...
      <div style={{ padding: '10px 12px', borderBottom: '1px solid #1e2a3a' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
          {PROFILES.map(p => {
            ...
            return (
              <span key={p.id} onClick={() => !isRunning && dispatch({ type: 'SET_PROFILE', profile: p.id })}
                style={{
                  padding: '3px 8px', borderRadius: 10, fontSize: 10, cursor: isRunning ? 'default' : 'pointer',
      ...
      {/* ── Run button ──────────────────────────────────────────────────── */}
      <div style={{ padding: '10px 12px', borderBottom: '1px solid #1e2a3a' }}>
        <button onClick={startResearch} disabled={isRunning || !state.query.trim()}
          style={{
            width: '100%', padding: '10px 0',
            background: isRunning ? '#1e3a5f'
              : (!state.query.trim() ? '#0d111a' : '#3b82f6'),
            color: '#fff', border: `1px solid ${isRunning ? '#1e3a5f' : '#3b82f6'}`,
            borderRadius: 6, fontFamily: 'Syne', fontWeight: 700,
            fontSize: 11, letterSpacing: 1, textTransform: 'uppercase',
            cursor: (isRunning || !state.query.trim()) ? 'not-allowed' : 'pointer',
            opacity: !state.query.trim() && !isRunning ? .3 : 1,
            transition: 'all .15s',
          }}>
          {isRunning
          ...
          return (
            <div key={id} style={{
              ...
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: color,
                flexShrink: 0, animation: ag.status === 'running' ? 'pulse 1s infinite' : 'none' }} />
              <span style={{ fontSize: 10, flex: 1, color, fontFamily: 'Space Mono' }}>
                {id.replace(/_/g, ' ')}
              </span>
              <span style={{ fontSize: 8, color, letterSpacing: 1, textTransform: 'uppercase' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5, padding: '10px 12px',
        ...
        {[
          ...
          <div key={label} style={{ background: '#131929', border: '1px solid #1e2a3a',
            ...
            <div style={{ fontFamily: 'Syne', fontSize: 16, fontWeight: 800, color }}>{val}</div>
            <div style={{ fontSize: 8, color: '#334155', letterSpacing: 1.5,
      ...
      {state.runComplete && state.finalBrief && (
        <div style={{ padding: '10px 12px' }}>
          <div style={{ padding: '6px 10px', background: 'rgba(16,185,129,.1)',
            border: '1px solid rgba(16,185,129,.3)', borderRadius: 4, fontSize: 10,
