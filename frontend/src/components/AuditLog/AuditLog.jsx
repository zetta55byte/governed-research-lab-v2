...
export default function AuditLog({ membraneLog, continuityChain }) {
  ...
  // Merge membrane checks + delta commits into one chronological log
  const entries = [
    ...
    return null;
              ...
              <div key={`m-${i}`} style={{
                ...
                <span style={{ fontSize: 9, fontWeight: 700, padding: '0 4px', borderRadius: 3,
                  background: sev.bg, color: sev.color, letterSpacing: 1, whiteSpace: 'nowrap' }}>
                  {sev.label}
                </span>
                <span style={{ fontSize: 9, color: MEMBRANE_COLORS[entry.membrane] || '#fff',
                  whiteSpace: 'nowrap' }}>
                  {entry.membrane?.split('_')[0]}
                </span>
                <span style={{ fontSize: 9, color: '#64748b', flex: 1, overflow: 'hidden',
